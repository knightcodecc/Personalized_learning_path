...
# Removed JWT protection to allow guest usage
...
from auth import hash_password, verify_password, validate_registration_data
...
# Ensure DB directory exists then create tables
...
def _guest_user():
	...
	if not user:
		user = User(username='guest', email='guest@local', password_hash=hash_password('guest'))
		db.session.add(user)
		db.session.commit()
	return user


# Auth Routes (kept minimal but without JWT requirements)
@app.post('/api/auth/register')
def register():
	# Disabled sign-up in guest mode
	return jsonify({'message': 'Sign-up disabled. Using guest mode.'}), 200


@app.post('/api/auth/login')
def login():
	# Disabled sign-in in guest mode
	guest = _guest_user()
	return jsonify({'message': 'Sign-in disabled. Using guest mode.', 'user': guest.to_dict()}), 200


@app.post('/api/auth/logout')
def logout():
	return jsonify({'message': 'Logged out (guest mode)'}), 200
...
def generate_path():
	...
	lp = LearningPath(
		user_id=user.id,
		title=f"{curriculum.get('title', 'Learning Path')} ({user_data['target_role']})",
